...
function AgreementForm({ defaultCompany, onSave }: { defaultCompany: string; onSave: (c: Contract) => void }) {
  const blankInst = (): Installment => ({ id: uid(), label: "قسط", due: today(), amount: 0, kind: "installment" });
  const [quantity, setQuantity] = useState(1); const [unitPrice, setUnitPrice] = useState(0); const [displayStart, setDisplayStart] = useState(today()); const [displayEnd, setDisplayEnd] = useState(""); const [mode, setMode] = useState<"auto" | "manual">("auto"); const [manual, setManual] = useState<Installment[]>([blankInst()]);
  const billingMonths = (start: string, end: string) => { if (!start || !end || end < start) return 0; const a = new Date(start + "T12:00:00"); const b = new Date(end + "T12:00:00"); const days = (y: number, m: number) => new Date(y, m + 1, 0).getDate(); let value = 0; if (a.getFullYear() === b.getFullYear() && a.getMonth() === b.getMonth()) value = (b.getDate() - a.getDate() + 1) / days(a.getFullYear(), a.getMonth()); else { value = (days(a.getFullYear(), a.getMonth()) - a.getDate() + 1) / days(a.getFullYear(), a.getMonth()) + b.getDate() / days(b.getFullYear(), b.getMonth()); const cursor = new Date(a.getFullYear(), a.getMonth() + 1, 1); const last = new Date(b.getFullYear(), b.getMonth(), 1); while (cursor < last) { value++; cursor.setMonth(cursor.getMonth() + 1); } } return Math.round(value * 10) / 10; };
  const months = billingMonths(displayStart, displayEnd);
  const total = quantity * unitPrice * months;
  function submit(e: React.FormEvent<HTMLFormElement>) { e.preventDefault(); const fd = new FormData(e.currentTarget); const agreementDate = String(fd.get("agreementDate")); const deposit = Number(fd.get("deposit") || 0); const depositDate = String(fd.get("depositDate") || agreementDate); if (!months || months > 12) { alert("اختر تاريخ نهاية بعد تاريخ البداية، وبحد أقصى 12 شهرًا."); return; } let installments: Installment[] = [];
    if (deposit > 0) installments.push({ id: uid(), label: "دفعة التعاقد", due: depositDate, amount: deposit, kind: "deposit" });
    ...
    onSave({ id: uid(), company: String(fd.get("company")), title: String(fd.get("title")), start: agreementDate, displayStart, quantity, monthlyUnitPrice: unitPrice, durationMonths: months, notes: String(fd.get("notes") || ""), signs: [{ id: uid(), location: String(fd.get("location")), start: displayStart, end: displayEnd, cost: total }], installments, payments: [] }); }
  return <form onSubmit={submit} className="form"><div className="form-grid"><label>شركة الإعلان<input name="company" required defaultValue={defaultCompany} readOnly={!!defaultCompany}/></label><label>اسم الإعلان / الحملة<input name="title" required placeholder="مثال: إعلان حج"/></label><label>موقع اليافطات<input name="location" required placeholder="مثال: شارع فؤاد"/></label><label>تاريخ الاتفاق<input name="agreementDate" type="date" required defaultValue={today()}/></label><label>تاريخ بداية عرض الإعلان<input type="date" required value={displayStart} onChange={e => setDisplayStart(e.target.value)}/></label><label>تاريخ نهاية عرض الإعلان<input type="date" required min={displayStart} value={displayEnd} onChange={e => setDisplayEnd(e.target.value)}/><small className="duration-result">المدة المحسوبة: {months || 0} شهر</small></label><label>عدد اليافطات<input type="number" min="1" required value={quantity} onChange={e => setQuantity(Number(e.target.value))}/></label><label>سعر الشهر لليافطة الواحدة<input type="number" min="1" required value={unitPrice || ""} onChange={e => setUnitPrice(Number(e.target.value))}/></label><label className="wide">ملاحظات<input name="notes"/></label></div><div className="calculation-box"><span>{quantity} يافطة</span><i>×</i><span>{money(unitPrice)} شهريًا</span><i>×</i><span>{months || 0} شهر</span><strong>{money(total)}</strong></div>
